...
export const docs: DocPage[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { title: 'The connection or tools do not appear', body: `<ol><li>For Claude Desktop or Continue, verify <code>node --version</code>.</li><li>Confirm the plugin-generated connector URL returns HTTP 200.</li><li>Restart or reload the client after saving configuration.</li><li>Check that the server entry is inside the exact root key the client expects.</li></ol>` },
      ...
      { title: 'Why an endpoint contains a legacy name', body: `<p>SiteRadian-generated endpoint URLs may contain <code>wp-command-center/v1</code>. This is a stable REST compatibility namespace retained by SiteRadian, not a different product or an MCP alias you need to configure. Use the exact generated endpoint while keeping the public MCP alias <code>siteradian</code> and environment variable <code>SITERADIAN_TOKEN</code>.</p>` },
